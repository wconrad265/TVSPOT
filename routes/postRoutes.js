...
module.exports = (app) => {
  ...
  app.post("/posts/:postId/edit",
    ...
    catchError(async (req, res) => {
      ...
      try {
        ...
        const editedPost = await res.locals.store.editPostTitle(postTitle, postId);
        if (!editedPost) {
          req.flash("error", "Forum Post title must be unique");
          return renderEditForumPost();
        }
  ...
  app.get("/posts/:postId", 
    ...
    catchError(async (req, res) => {
      ...
      const maxPageNumber = await res.locals.store.getMaxComments(postId, Comments_PER_PAGE);
      let comments = [];

      if ((maxPageNumber === 0 && pageNumber > 1) || (maxPageNumber !== 0 && pageNumber > maxPageNumber)) {
