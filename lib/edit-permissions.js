...
const checkEditPostPermissions = catchError(async (req, res, next) => {
  const resourceUserId = await res.locals.store.getPostUserId(+req.params.postId);
...
const checkEditCommentPermissions = catchError(async (req, res, next) => {
  const resourceUserId = await res.locals.store.getCommentUserId(+req.params.commentId);
