...
module.exports = class PGPersistence {
  ...
  async getMaxPosts(postsPerPage) {
    ...
    const result = await dbQuery(TOTAL_POSTS);
  ...
  async getMaxComments(postId, postsPerPage) {
    ...
    const result = await dbQuery(TOTAL_COMMENTS, postId);
  ...
  async createPost(postTitle) {
    const CREATE_POST = 'INSERT INTO posts (title, user_id) VALUES ($1, $2) RETURNING id';

    try {
      const result = await dbQuery(CREATE_POST, postTitle, this.userId);
  ...
  async createComment(postId, commentText) {
    const CREATE_COMMENT = 'INSERT INTO  comments (post_id, markdown_body, user_id) VALUES ($1, $2, $3)';

    const result = await dbQuery(CREATE_COMMENT, postId, commentText, this.userId);
  ...
  async getCommentsForPage(postId, pageNumber, commentsPerPage) {
    ...
    const result = await dbQuery(GET_COMMENTS, postId, commentsPerPage, startIndex);

    return result.rows;
  }

  async getComment(commentId) {
    const GET_COMMENT = "select markdown_body FROM comments WHERE id = $1"

    const result = await dbQuery(GET_COMMENT, commentId);

    return result.rows[0].markdown_body;
  }

  async updateComment(comment, commentId) {
    const UPDATE_COMMENT = "UPDATE comments SET markdown_body = $1 WHERE id = $2 AND user_id = $3";

    const result = await dbQuery(UPDATE_COMMENT, comment, commentId, this.userId);
  ...
  async deleteComment(post_id, commentId) {
    ...
    const result = await dbQuery(DELETE_COMMENT, commentId, post_id);
  ...
  async getPostTitle(postId) {
    ...
    const result = await dbQuery(GET_POST_TITLE, postId);
  ...
  async getUserId(username) {
    ...
    const result = await dbQuery(FIND_USER_ID, username);
  ...
  async getUserPosts() {
    ...
    const result = await dbQuery(GET_USER_POSTS, this.userId);
  ...
  async deletePost(postId) {
    ...
    const result = await dbQuery(DELETE_POST, postId);

    return result.rowCount > 0;
  }

  async existsForumPostTitle(postTitle) {
    const FIND_POST_TITLE = 'SELECT null from posts WHERE title = $1';

    const result = await dbQuery(FIND_POST_TITLE, postTitle);

    return result.rowCount > 0;
  }

  async editPostTitle(postTitle, postId) {
    const UPDATE_POST = 'UPDATE posts SET title = $1 WHERE id = $2 AND user_id = $3';

    const result = await dbQuery(UPDATE_POST, postTitle, postId, this.userId);

    return result.rowCount > 0;
  }

  async getPostUserId(postId) {
    const GET_POST_USER_ID = "SELECT user_id FROM posts WHERE ID = $1";

    const result = await dbQuery(GET_POST_USER_ID, postId);

    return result.rows[0].user_id;
  }

  async getCommentUserId(commentId) {
    const GET_COMMENT_USER_ID = "SELECT user_id FROM comments WHERE ID = $1";

    const result = await dbQuery(GET_COMMENT_USER_ID, commentId);
