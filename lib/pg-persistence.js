...
module.exports = class PGPersistence {
  ...
  async getMaxPosts(postsPerPagination) {
    ...
    const maxPosts = result.rowCount === 0 ? 1 : result.rowCount; //set to 1 if their are no posts
  ...
  async getMaxComments(postId, commentsPerPagination) {
    ...
    const maxComments = result.rowCount === 0 ? 1 : result.rowCount; //set to 1 if there are no comments

    return Math.ceil(maxComments / commentsPerPagination);
  }

  async getMaxUserPosts(postsPerPagination) {
    const SELECT_ALL_USER_POSTS = "SELECT * FROM posts WHERE user_id = $1";

    const result = await dbQuery(SELECT_ALL_USER_POSTS, this.userId);

    const maxUserPosts = result.rowCount === 0 ? 1 : result.rowCount; //set to 1 is users have no posts.

    return Math.ceil(maxUserPosts / postsPerPagination);
  }

  async getUserPostsForPage(pageNumber, postsPerPagination) {
    ...
    const SELECT_POSTS = `
      SELECT
        ...
        CASE
          ...
          ELSE to_char(p.date, 'YYYY-MM-DD')
        END AS formatted_date,
        TO_CHAR(p.date, 'HH12:MI AM TZ') AS time,
        (SELECT username FROM users u WHERE u.id = p.user_id) AS username,
        (SELECT count(id) FROM comments c WHERE c.post_id = p.id) AS number_of_comments
      FROM posts p
      WHERE user_id = $1
  ...
  async getCommentsForPage(postId, pageNumber, commentsPerPagination) {
    ...
    const SELECT_COMMENTS = `
      SELECT 
        ...
        c.date,
        CASE
          WHEN c.date::date = CURRENT_DATE THEN 'today'
          WHEN c.date::date = CURRENT_DATE - interval '1 day' THEN 'yesterday'
          ELSE to_char(c.date, 'YYYY-MM-DD')
        END AS formatted_date,
        TO_CHAR(c.date, 'HH12:MI AM TZ') AS time,
